...
df1 <- read.csv("D:\\RStudio\\CUNY_622\\2\\Titanic.csv")
...
df4 <- df2
set.seed(042760)
i <- createDataPartition(df4$Survived, p=0.80, list=FALSE)
dfEval <- df4[-i,]
dfTrain <- df4[i,]

dfTrain %>% count(Survived)

tc <- trainControl(method="cv", number=10)
metric <- "Accuracy"

set.seed(042760)
#multinom <- train(Survived~., data=dfTrain, method="multinom", metric=metric, trControl=tc)
rf <- train(Survived~., data=dfTrain, method="rf", metric=metric, trControl=tc)
summary(multinom)
results <- resamples(list(Multinomial=multinom, RandomForest=rf))
summary(results)
dotplot(results)
...
dt <- train(Survived~., data=dfTrain, method="rpart", metric=metric, trControl=tc)
library(rpart.plot)
rpart.plot(dt$finalModel)
```
