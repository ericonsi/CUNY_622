...
df3 <- df2 %>%
  mutate(highPT = ifelse(ptratio>18.4, 1, 0)) %>%
  dplyr::select(-ptratio)

#devtools::install_github("ericonsi/EHData")
#library(EHData)
c <- EHModel_DecisionTree(df3, "highPT")
c <- EHModel_SVM(df3, "highPT", method="radial", printPlot=FALSE)
d <- EHModel_RandomForest(df3, "highPT", categorical=TRUE)
e <- EHModel_Regression_Logistic(df3, "highPT")
