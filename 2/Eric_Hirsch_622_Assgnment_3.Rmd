...
df3 <- df2 %>%
  ...
  dplyr::select(highPT, indus, medv)
...
ggplot(df3a, aes(x=indus, y=medv, color=highPT)) +
  geom_point()
...
library(EHData)
library("caret")
q <- EHModel_DecisionTree(df3c, "highPT", levels=4)
c <- EHModel_SVM(df3c, "highPT", method="radial", printPlot=FALSE)
d <- EHModel_RandomForest(df3c, "highPT", categorical=TRUE, printPlot=FALSE, printVarimp = TRUE)
e <- EHModel_Regression_Logistic(df3c, "highPT")
