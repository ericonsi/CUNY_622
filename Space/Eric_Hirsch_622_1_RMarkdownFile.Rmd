...
dfTrain_MissingRemoved <- na.omit(dfTrain)
dfTrain_MissingRemoved_Small <- dfTrain_MissingRemoved %>%
 filter(row_number() %% 30 == 1) %>%
  dplyr::select(-PassengerId, -Cabin, -Destination, -Name)

dfx <- EHPrepare_CreateDummies(dfTrain_MissingRemoved_Small, 'Transported')

EHModel_RandomForest(dfx, "Transported")
