...
dfTest1 <- EHPrepare_MissingValues_Imputation(dfTest, impute="median")
```
...
#dfTrain2$Group <- sub("\\_.*", "", dfTrain2$PassengerId)
...
Group2 <- Group2 %>% mutate(Transported=ifelse(Transported=='True',1,0))

Group2$Group <- sub("\\_.*", "", Group2$PassengerId)

Group2Sum <- Group2 %>%
  group_by(Group) %>%
  summarize(PredPercent= sum(Transported)/n(), count=n())
...
Group2Sum$Determination <- ifelse(Group2Sum$PredPercent>.5, "True", ifelse(Group2Sum$PredPercent==.5, "None", "False"))

new <- as.data.frame(lapply(result, function(x) Group2Sum$Determination[match(x, Group2Sum$Group)]))
...
dfTrain5a <- dfTrain5 %>%
  mutate(Tot = VRDeck + ShoppingMall + Spa + RoomService + FoodCourt) %>%
  dplyr::select(-Spa)
...
test_x <- dfTest6 %>%
  dplyr::select(-Transported,-PassengerID)
test_y <- dfTest6 %>%
  dplyr::select(Transported)
...
#tRIAL AND ERROR GAVE 55 AS BEST- COULD TRY OTHERS
